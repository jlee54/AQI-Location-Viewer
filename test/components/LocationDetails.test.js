...
test("When the location is User IP Based", async () => {
  ...
  const mockFetchUserCity = vi.spyOn(LocationDetails.methods, "fetchUserCity").mockReturnValue(JSON.parse(UserIpLocationInfoJson()).city)
...
test("When the location has been cached", async () => {
  localStorage.setItem("aqi_details", JSON.stringify({}))
...
// Wanted to do this with Fake Timers, to test data_creation_date again
// but there were issues in configuration
...
// Todo: Cover other aqi categorization scenarios for full test coverage
...
function UserIpLocationInfoJson() {
